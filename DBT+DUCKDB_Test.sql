...
-- Ce projet met en place plusieurs tests personnalisés (custom tests) exécutés via dbt sur DuckDB afin de garantir l’intégrité des données,
-- leur cohérence statistique et le respect des règles métier sur le modèle transform (dataset de trajets de taxis de NYC).

-- Contrairement aux tests standards de dbt (not_null, unique, etc.), ces contrôles visent à détecter des anomalies spécifiques au métier ainsi que des erreurs
-- structurelles susceptibles de biaiser fortement les analyses en aval.
